...
class BowTieRanker(Ranker):
	""" Ranks by the Bow Tie weighted overlap metric between any two ids (focal nodes)"""

	def __str__(self):
		return "BowTieRanker"

	def _rank(self, data_pair, top_n, survey_time):
		data, target_id = data_pair

		edge_list = []
		for id, interactions in data.items():
			event_count = {
			...
			if len(event_count) > 0:
				for k, v in event_count.items():
					edge_list.append((int(id), k, {'weight': v}))
		...
		if i in G:
			...
			for j in list(data[target_id].keys()):
				if j in G and G.has_edge(i, j):
					w_ij = G[i][j]['weight']

					g_j = set(G.neighbors(j))
					s_j = 0
					for k in g_j:
						s_j += G[j][k]['weight']	

					g_ij = list(g_i.intersection(g_j))
					if i in g_ij:
						g_ij.remove(i)
					if j in g_ij:
						g_ij.remove(j)
					...
					denominator = s_i + s_j - 2 * w_ij
					if denominator > 0:
						weighted_o = numerator / denominator
						res[j] = weighted_o
					else:
						res[j] = 0
				else:
	...
	def get_signals(self, ids, users_interaction_dict, times, norm_all=True):
		raise Exception("BowTieRanker does not have a get_signals() method")
...
class FreqRanker(Ranker):
	""" Ranks by number of events divided by duration. """
...
class DurationRanker(Ranker):
	""" Ranks by longest history of interaction"""

	def __str__(self):
		return "DurationRanker"

	def _rank(self, data, top_n, survey_time):
		""" ranks the top_n possible ids in data """
		most_recent_timestamp = {
			k: min(v[np.asarray(v[:, 2] <= survey_time)][:, 2])
			for k, v in data.items()
			if np.any(v[:, 2] <= survey_time)
		}
...
class PairwiseRanker(Ranker):
	...
	def _create_indiv_feat_vec(self, indiv_data, survey_time):
		...
		feat_vec = [
			...
			get_freq(events, survey_time),
			get_duration(events, survey_time),
...
class PairwiseRankerSplit(PairwiseRanker):
	""" 
	Ranks using a Comparer, cannot use TimeSeriesComparers

	Same as PairwiseRanker but splits call and text
	...
	def __str__(self):
		if self.desc_str is None:
			return "PairwiseRankerSplit {}: {}".format(
				self.rank_method, str(self.comparer))
		else:
			return self.desc_str
	...
	def _create_indiv_feat_vec(self, indiv_data, survey_time):
		...
		Features (in order they are in resulting array):
			...
			hawkes process signal: at survey time using given params
		"""
		calls = indiv_data[ indiv_data[:, 0] == 0 ][:, 2]
		texts = indiv_data[ indiv_data[:, 0] == 1 ][:, 2]

		feat_vec = [
			get_volume(calls, survey_time),
			get_recency(calls, survey_time),
			get_freq(calls, survey_time),
			get_duration(calls, survey_time),
			get_volume(texts, survey_time),
			get_recency(texts, survey_time),
			get_freq(texts, survey_time),
			get_duration(texts, survey_time)
		]
