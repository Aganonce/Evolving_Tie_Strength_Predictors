...
from models.comparers import TimeSeriesComparerNoScaler
...
def get_freq(events, survey_time):
	if len(events) == 0:
		return 0
	return len(events) / (survey_time - min(events))


def get_duration(events, survey_time):
	if len(events) == 0:
		return 0
	return survey_time - min(events)
...
def plot_rankers_grid(rankers, interactions, surveys, id_to_plot,
	...
	start_time = min( min(all_event_times), min(survey_times) ) - 3600 * 24
	end_time = max( max(all_event_times), max(survey_times) ) + 3600 * 24
